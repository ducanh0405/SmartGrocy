...
class SensitivityAnalyzer:
    ...
    def analyze_by_product_group(
        self, df: pd.DataFrame, metrics: List[str] = ["mae", "rmse", "r2_score"]
    ) -> pd.DataFrame:
        """
        ...
        for name, group in groups:
            if "actual" in group.columns and "predicted" in group.columns:
                ...
                r2 = 1 - (
                    np.sum((group["actual"] - group["predicted"]) ** 2)
    ...
    def analyze_by_region(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for name, group in groups:
            if "actual" in group.columns and "predicted" in group.columns:
                mae = np.mean(np.abs(group["actual"] - group["predicted"]))
                mape = (
                    np.mean(np.abs((group["actual"] - group["predicted"]) / group["actual"])) * 100
                )
            else:
                mae, mape = 0, 0
            ...
            results.append(
                {
                    "region": name,
                    "n_stores": (
                        len(group["store_id"].unique()) if "store_id" in group.columns else 1
                    ),
                    "n_predictions": len(group),
                    "mae": mae,
                    "mape": mape,
    ...
    def scenario_analysis(
        self, df: pd.DataFrame, scenarios: Dict[str, Dict] = None
    ) -> pd.DataFrame:
        """
        ...
        for scenario_name, params in scenarios.items():
            ...
            if "predicted" in df_scenario.columns:
                df_scenario["predicted_adj"] = df_scenario["predicted"] * (
                    1 + params["demand_change"]
                )
            ...
            if "predicted_adj" in df_scenario.columns and "price_adj" in df_scenario.columns:
                revenue_baseline = (
                    (df["predicted"] * df["price"]).sum() if "price" in df.columns else 0
                )
                revenue_scenario = (df_scenario["predicted_adj"] * df_scenario["price_adj"]).sum()
                revenue_change = (
                    ((revenue_scenario - revenue_baseline) / revenue_baseline * 100)
            ...
            results.append(
                {
                    ...
                    "avg_demand": (
                        df_scenario["predicted_adj"].mean()
                        if "predicted_adj" in df_scenario.columns
                        else 0
                    ),
                }
            )
    ...
    def generate_sensitivity_report(
        self, predictions_df: pd.DataFrame, output_dir: str = "reports/sensitivity"
    ):
        """
