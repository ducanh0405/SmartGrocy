...
def compute_shap_values(
    sample_size: int = None, quantile: float = 0.50, model_type: str = "lightgbm"
) -> tuple:
    """
    ...
    try:
        X_sample, shap_summary = forecaster.predict_shap(
            df_test, model_type=model_type, sample_size=sample_size, quantile=quantile
        )
...
def main():
    ...
    parser.add_argument(
        "--quantile", type=float, default=0.50, help="Quantile to analyze (default: 0.50 = median)"
    )
    parser.add_argument(
        "--model-type",
        type=str,
    ...
    try:
        ...
        X_sample, shap_summary = compute_shap_values(
            sample_size=args.sample_size, quantile=args.quantile, model_type=args.model_type
        )
