...
def run_backtesting_simulation(predictions_df):
    ...
    daily_data = (
        predictions_df.groupby(["product_id", "date"])
        .agg({"actual_demand": "sum", "pred_q50": "sum", "pred_q95": "sum"})
        .reset_index()
    )

    daily_data.rename(
        columns={"pred_q50": "forecast_q50", "pred_q95": "forecast_q95"}, inplace=True
    )
    ...
    top_products = daily_data.groupby("product_id")["actual_demand"].sum().nlargest(50).index
    ...
    backtester = InventoryBacktester(
        historical_demand=backtest_data, forecasts=backtest_data, config=backtest_config
    )
...
def run_estimation_fallback():
    ...
    baseline_profit = (
        4.3  # Net profit margin AFTER losses (Revenue - COGS - Ops - Spoilage - Stockout)
    )
    ...
    logger.info(
        f"  Based on: R² × 0.50 for spoilage, R² × 0.35 for stockout (conservative literature conversion)"
    )
        ...
        try:
            ...
            q50_col = next(
                (
                    ...
                    if "q50" in str(col).lower() or "q_50" in str(col).lower()
                ),
                None,
            )
            q95_col = next(
                (
                    col
                    for col in predictions_df.columns
                    if "q95" in str(col).lower() or "q_95" in str(col).lower()
                ),
                None,
            )
            ...
            if q50_col and q95_col:
                ...
                predictions_df.rename(
                    columns={q50_col: "pred_q50", q95_col: "pred_q95"}, inplace=True
                )

                # Get actual demand column
                actual_col = (
                    "sales_quantity" if "sales_quantity" in predictions_df.columns else "actual"
                )
