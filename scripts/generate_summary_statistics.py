...
def calculate_inventory_summary() -> Dict:
    ...
    summary = {
        ...
        "products_need_reorder": (
            df["should_reorder"].sum() if "should_reorder" in df.columns else 0
        ),
        "avg_stockout_risk": (
            df["stockout_risk_pct"].mean() if "stockout_risk_pct" in df.columns else 0
        ),
        "avg_safety_stock": df["safety_stock"].mean() if "safety_stock" in df.columns else 0,
        "high_risk_products": (
            (df["stockout_risk_pct"] > 50).sum() if "stockout_risk_pct" in df.columns else 0
        ),
    }
...
def calculate_pricing_summary() -> Dict:
    ...
    summary = {
        ...
        "products_with_discount": (
            (df["discount_pct"] > 0).sum() if "discount_pct" in df.columns else 0
        ),
        "avg_discount_pct": df["discount_pct"].mean() * 100 if "discount_pct" in df.columns else 0,
        "total_revenue_impact": (
            df["expected_revenue_impact"].sum() if "expected_revenue_impact" in df.columns else 0
        ),
    }
...
def main():
    ...
    summaries["aggregate"] = {
        ...
        "reorder_recommendations": summaries["inventory_optimization"].get(
