...
def generate_predictions_distribution():
    ...
    ax.axvline(np.mean(forecasts), color='red', linestyle='--', label=f'Mean: {np.mean(forecasts):.1f}')
    ax.axvline(np.median(forecasts), color='green', linestyle='--', label=f'Median: {np.median(forecasts):.1f}')
    ax.legend()
