...
def create_expectation_suite(context):
    ...
    for feature in all_features:
        expectations.append(
            {"expectation_type": "expect_column_to_exist", "kwargs": {"column": feature}}
        )
        ...
        expectations.append(
            {
                ...
                "kwargs": {"column": feature, "type_": "float64"},
            }
        )
        ...
        expectations.append(
            {
                ...
                "kwargs": {"column": feature, "mostly": 0.90},
            }
        )
    ...
    for feature in categorical_features[:10]:  # Limit to first 10
        expectations.append(
            {
                "expectation_type": "expect_column_values_to_not_be_null",
                "kwargs": {"column": feature, "mostly": 0.85},  # Allow 15% nulls for categorical
            }
        )
...
def test_validation():
    ...
    if not master_table_path.exists():
        ...
        logger.warning(
            "  Run feature engineering first: python src/pipelines/_02_feature_enrichment.py"
        )
    ...
    try:
        ...
        if stats:
            for run_id, run_result in stats.items():
                ...
                logger.info(
                    f"    Failed expectations: {statistics.get('unsuccessful_expectations', 0)}"
                )
