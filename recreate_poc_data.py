...
def recreate_poc_data():
    ...
    print("RECYCLING: RECREATING POC DATA WITH PRODUCT_ID MATCHING")
    print("=" * 60)
    ...
    if not transaction_path.exists():
        print(f"ERROR: Transaction data not found: {transaction_path}")
        return False
    ...
    print("\nPRODUCT_ID Analysis:")
    print(f"   Transaction products: {len(trans_products):,}")
    print(f"   Product catalog: {len(product_ids):,}")
    print(f"   Matching products: {len(valid_products):,}")
    print(f"   Match rate: {len(valid_products)/len(trans_products)*100:.1f}%")
    if len(valid_products) == 0:
        print("ERROR: No matching products! Cannot create valid POC data.")
        return False
    ...
    print("\nCreating POC Sample:")
    print(f"   Sample size: {len(sampled_products):,} products ({len(sampled_products)/len(valid_products)*100:.1f}%)")
    ...
    # Save samples
    print("\nSaving POC data...")
    df_trans_sample.to_csv(poc_dir / 'transaction_data.csv', index=False)
    ...
    print("\nSUCCESS: POC Data Recreation Complete!")
    print(f"   Location: {poc_dir}")
    ...
    print("\nNext Steps:")
    print("   1. Run: python scripts/run_optimized_pipeline.py")
...
if __name__ == "__main__":
    ...
    if not success:
        print("\nERROR: POC data recreation failed!")
        print("TIP: Ensure data/2_raw/ contains transaction_data.csv and product.csv")
        sys.exit(1)
