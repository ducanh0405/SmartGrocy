...
class TestDataPipelineIntegration:
    ...
    def test_pipeline_data_consistency(self, sample_freshretail_data):
        ...
        assert (
            len(master_df) >= expected_min_rows
        ), f"Too few rows: {len(master_df)} < {expected_min_rows}"
        ...
        assert (
            master_products == original_products
        ), f"Lost products: {master_products} != {original_products}"
        assert (
            master_stores == original_stores
        ), f"Lost stores: {master_stores} != {original_stores}"
