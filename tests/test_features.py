...
def sample_master_data(sample_transaction_data):
    ...
    master = (
        sample_transaction_data.groupby(["PRODUCT_ID", "STORE_ID", "WEEK_NO"])
        .agg(
            {
                "SALES_VALUE": "sum",
                "QUANTITY": "sum",
...
class TestWS2TimeSeriesFeatures:
    ...
    def test_create_lag_features(self, sample_master_data):
        ...
        result = create_lag_features(sample_master_data, target_col="SALES_VALUE", lags=[1, 4])
    ...
    def test_create_rolling_features(self, sample_master_data):
        ...
        result = create_rolling_features(
            df_with_lags, target_col="SALES_VALUE", base_lag=1, windows=[4]
        )
    ...
    def test_add_intraday_features(self, sample_master_data):
        ...
        hourly_data["hour_timestamp"] = pd.date_range(
            "2024-01-01", periods=len(hourly_data), freq="h"
        )
    ...
    def test_add_trend_features(self, sample_master_data):
        ...
        assert (
            "wow_change" in result.columns
            or "momentum" in result.columns
            or "volatility" in result.columns
        )
    ...
    def test_add_interaction_features(self, sample_master_data):
        ...
        interaction_cols = [
            col for col in result.columns if "interaction" in col.lower() or "ratio" in col.lower()
        ]
    ...
    def test_rolling_features_with_multiple_windows(self, sample_master_data):
        ...
        result = create_rolling_features(
            df_with_lags, target_col="SALES_VALUE", base_lag=1, windows=[4, 8, 12]
        )
    ...
    def test_calendar_features_hourly_data(self):
        ...
        assert (
            "day_of_year" in result.columns
            or "month" in result.columns
            or "quarter" in result.columns
        )
...
class TestWS4PriceFeatures:
    ...
    def test_price_features_calculation(self, sample_master_data):
        ...
        expected_base = result["SALES_VALUE"] - (result["RETAIL_DISC"] + result["COUPON_DISC"])
        np.testing.assert_allclose(result["base_price"], expected_base, rtol=1e-5)
