...
def test_ws0_aggregation(sample_freshretail_data):
    ...
    is_sorted = (
        master_df.groupby(["product_id", "store_id"])["hour_timestamp"]
...
def test_ws2_timeseries_features(sample_freshretail_data):
    ...
    try:
        ...
        expected_lag_cols = [
            f"sales_quantity_lag_{lag}" for lag in config.get("lag_periods", [1, 24, 48, 168])
        ]
        found_lags = [
            col for col in expected_lag_cols[:2] if col in enriched_df.columns
        ]  # Check first 2 lags
        assert (
            len(found_lags) > 0
        ), f"Missing expected lag features. Expected: {expected_lag_cols[:2]}, Found: {list(enriched_df.columns)}"

        # Verify rolling features exist (if configured)
        if config.get("rolling_windows"):
            rolling_cols = [
                col for col in enriched_df.columns if "rolling_mean" in col or "rolling_std" in col
            ]
        ...
        if config.get("has_intraday_patterns", False):
            assert (
                "hour_of_day" in enriched_df.columns or "hour_sin" in enriched_df.columns
            ), "FreshRetail should have intraday pattern features"
            assert (
                "day_of_week" in enriched_df.columns or "dow_sin" in enriched_df.columns
            ...
            if lag_cols:
                for lag_col in lag_cols[:1]:  # Check first lag column
                    if lag_col in first_periods.columns:
                        ...
                        assert (first_periods[lag_col] == 0).all() or first_periods[
                            lag_col
                        ].isna().all(), f"Lag feature {lag_col} should be 0 or NaN for first period (no future data leak)"
...
def test_time_based_split():
    ...
    assert (
        train["hour_timestamp"].max() < test["hour_timestamp"].min()
...
def test_quantile_model_config():
    ...
    model_q50 = lgb.LGBMRegressor(
        objective="quantile", alpha=0.50, n_estimators=10, random_state=42
    )
