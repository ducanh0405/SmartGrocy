...
MODEL_TYPES = [
    "lightgbm"
]  # Mặc định chỉ train LightGBM để test nhanh. Có thể thêm: ['lightgbm', 'catboost', 'random_forest']
...
DATA_QUALITY_CONFIG = {
    ...
    "quality_thresholds": {"excellent": 95, "good": 85, "fair": 70, "poor": 50},
    "drift_thresholds": {
...
PERFORMANCE_CONFIG.update(
    {
        ...
        "retry_config": {"max_retries": 3, "retry_delay_seconds": 60, "exponential_backoff": True},
    }
)
...
def get_model_config(quantile: float, model_type: str = "lightgbm") -> dict[str, Any]:
    ...
    if model_type not in MODEL_CONFIGS:
        raise ValueError(
            f"Model type '{model_type}' not supported. Available: {list(MODEL_CONFIGS.keys())}"
        )
...
def setup_logging(level: str = None, log_to_file: bool = None) -> None:
    ...
    log_to_file = (
        log_to_file if log_to_file is not None else LOGGING_CONFIG.get("log_to_file", True)
    )  # pyright: ignore[reportUndefinedVariable]
...
def validate_dataset_config(config: dict[str, Any]) -> list[str]:
    ...
    if "temporal_unit" in config:
        ...
        if config["temporal_unit"] not in valid_units:
            errors.append(
                f"Invalid temporal_unit: {config['temporal_unit']}. Must be one of: {valid_units}"
            )
