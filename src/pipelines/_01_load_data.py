...
def _sample_data_for_memory_optimization(
    ...
    if "sales" in dataframes and dataframes["sales"] is not None:
        ...
        if not isinstance(sample_fraction, int | float):
            logger.error(
                f"Invalid sample_fraction type: {type(sample_fraction)}. Using default 0.1"
            )
            sample_fraction = 0.1
        if sample_fraction <= 0 or sample_fraction > 1.0:
            logger.error(
                f"Invalid sample_fraction value: {sample_fraction}. Must be 0 < fraction <= 1.0. Using default 0.1"
            )
            sample_fraction = 0.1
        if sample_fraction < 1.0:
            df = df.sample(frac=sample_fraction, random_state=42).reset_index(drop=True)
            logger.info(
                f"  Sampled {original_size:,} -> {len(df):,} rows ({sample_fraction*100:.1f}%)"
            )
...
def _clean_raw_data(dataframes: dict[str, pd.DataFrame], config: dict) -> dict[str, pd.DataFrame]:
    ...
    if "sales" in dataframes and dataframes["sales"] is not None:
        ...
        if target_col and target_col in df.columns:
            ...
            if len(df) < original_rows:
                logger.warning(
                    f"  Removed {original_rows - len(df)} rows with negative {target_col}."
                )
        else:
            logger.warning(
                f"  Target column '{target_col}' not found in sales data, skipping filtering."
            )
    ...
    if "weather" in dataframes and dataframes["weather"] is not None:
        ...
        if "temperature" in df_weather.columns:
            ...
            df_weather["temperature"] = df_weather["temperature"].clip(
                -20, 50
            )  # Giả định khoảng -20°C đến 50°C
            clipped_count = (
                (df_weather["temperature"] == -20) | (df_weather["temperature"] == 50)
            ).sum()
            if clipped_count > 0:
                logger.info(
                    f"  Clipped {clipped_count} temperature values (range: {original_min:.1f} to {original_max:.1f})"
                )
        ...
        if "humidity" in df_weather.columns:
            ...
            if clipped_count > 0:
                logger.info(
                    f"  Clipped {clipped_count} humidity values (range: {original_min:.1f} to {original_max:.1f})"
                )
...
def _load_freshretail_data(data_dir: Path) -> dict[str, pd.DataFrame]:
    ...
    if stockout_df is None:
        ...
        sales_df = sales_df.merge(
            stockout_df[merge_keys + ["is_stockout"]], on=merge_keys, how="left"
        )
...
def load_data() -> tuple[dict[str, pd.DataFrame], dict]:
    ...
    logger.info(
        f"✅ Data loading complete. Loaded {len(dataframes)} dataframes: {list(dataframes.keys())}"
    )
