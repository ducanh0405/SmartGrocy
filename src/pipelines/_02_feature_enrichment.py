...
def main() -> None:
        ...
        if "weather" in dataframes and dataframes["weather"] is not None:
            try:
                ...
                with performance_monitor.time_operation(
                    "WS6_weather_features", {"has_weather": True}
                ):
    ...
    if config["has_behavior"]:
        if "clickstream_log" in dataframes and dataframes["clickstream_log"] is not None:
            ...
            logger.warning(
                "SKIPPING WS3: 'has_behavior' is True, but 'clickstream_log' data not found."
            )
    else:
    ...
    if config["has_price_promo"]:
        if "causal_data" in dataframes and dataframes["causal_data"] is not None:
            ...
            logger.warning(
                "SKIPPING WS4: 'has_price_promo' is True, but 'causal_data' data not found."
            )
    else:
    ...
    try:
        ...
        if num_rows > 1000000:
            logger.warning(
                f"Large dataset detected ({num_rows:,} rows). CSV export may take several minutes..."
            )
        ...
        logger.info(
            f"STATS: Final Shape: {master_df.shape[0]:,} rows, {master_df.shape[1]} columns"
        )
