...
def create_master_dataframe(
    dataframes: dict[str, pd.DataFrame], dataset_config: dict[str, Any]
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        raise PipelineError(
            f"Master dataframe creation failed: {e}",
            stage="create_master_dataframe",
            original_error=e,
        )


@task(retries=2, retry_delay_seconds=30)
def enrich_features(master_df: pd.DataFrame, dataset_config: dict[str, Any]) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if alert_manager:
            alert_manager.alert_pipeline_failure(
                pipeline_stage="feature_enrichment", error_message=str(e)
            )
        raise PipelineError(
            f"Feature enrichment failed: {e}", stage="enrich_features", original_error=e
        )


@task(retries=1)
def train_models(enriched_df: pd.DataFrame, dataset_config: dict[str, Any]) -> dict[str, Any]:
    """
    Train models.
...
def generate_quality_summary(report_path: Path, data_docs_path: Path | None) -> None:
    ...
    if model_metrics_path.exists():
        try:
            ...
            report_lines.append(
                f"R² Score: {r2:.4f}" if isinstance(r2, int | float) else f"R² Score: {r2}"
            )

            # Coverage
            coverage = metrics.get("coverage_90%", "N/A")
            report_lines.append(
                f"Coverage (90%): {coverage:.2%}"
                if isinstance(coverage, int | float)
                else f"Coverage (90%): {coverage}"
            )
            ...
            report_lines.append(
                f"MAE (Q50): {q50_mae:.4f}"
                if isinstance(q50_mae, int | float)
                else f"MAE (Q50): {q50_mae}"
            )
            report_lines.append(
                f"RMSE (Q50): {q50_rmse:.4f}"
                if isinstance(q50_rmse, int | float)
                else f"RMSE (Q50): {q50_rmse}"
            )
            ...
            if q50_mape is not None:
                if isinstance(q50_mape, int | float) and q50_mape < 1000:
                    ...
                    report_lines.append(
                        f"MAPE (Q50): Calculated with threshold (valid: {mape_valid}/{mape_total} samples)"
                    )
            else:
    ...
    if validation_path.exists():
        try:
            ...
            report_lines.append(
                f"Validation Status: {'✅ PASSED' if passed else '⚠️ FAILED (see issues below)'}"
            )
            ...
            if missing and "percentages" in missing:
                ...
                if high_missing:
                    ...
                    report_lines.append(
                        "  (This is expected for lag features at the start of time series)"
                    )
    ...
    if data_docs_path and data_docs_path.exists():
        ...
        report_lines.append(
            "GX Data Docs: Not generated (run: python scripts/setup_great_expectations.py)"
        )

    report_lines.append(f"Model Metrics: {OUTPUT_FILES['model_metrics']}")
    report_lines.append(
        f"Business Report: {OUTPUT_FILES['reports_dir'] / 'business_report_detailed.csv'}"
    )
...
def generate_quality_report():
    ...
    try:
        ...
        raise PipelineError(
            f"Quality report generation failed: {e}",
            stage="generate_quality_report",
            original_error=e,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full-data", action="store_true", help="Use full dataset")
