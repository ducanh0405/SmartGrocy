...
class RobustImputer:
    ...
    def _drop_high_missing_cols(self, df: pd.DataFrame, missing_report: dict) -> pd.DataFrame:
        ...
        cols_to_drop = [
            col for col, rate in missing_report.items() if rate > self.config.drop_threshold
        ]
        ...
        for col, rate in missing_report.items():
            if self.config.proxy_threshold < rate <= self.config.drop_threshold:
                if col in df.columns:
                    ...
                    df[proxy_col] = df[col].interpolate(method="linear", limit_direction="both")

                    # If still has missing, use rolling mean
                    if df[proxy_col].isna().any():
                        df[proxy_col] = df[proxy_col].fillna(
                            df[proxy_col]
                            .rolling(window=self.config.rolling_window, min_periods=1)
                            .mean()
                        )
    ...
    def _impute_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        vol_cols = [
            col for col in df.columns if "std" in col.lower() or "volatility" in col.lower()
        ]
            ...
            for col in vol_cols:
                if df[col].isna().any():
                    ...
                    df[col] = df[col].fillna(
                        df[col].rolling(window=self.config.rolling_window, min_periods=1).median()
                    )
    ...
    def _impute_change_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        change_cols = [
            col for col in df.columns if "change" in col.lower() or "diff" in col.lower()
        ]
    ...
    def _impute_rolling_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        rolling_cols = [col for col in df.columns if "rolling_mean" in col.lower()]
    ...
    def _validate_imputation(self, df_before: pd.DataFrame, df_after: pd.DataFrame) -> dict:
        ...
        report["improvement"] = (
            ...
            / report["missing_rate_before"]
            if report["missing_rate_before"] > 0
            else 0
        )
    ...
    def _log_validation_report(self, report: dict):
        ...
        logger.info(
            f"Missing rate: {report['missing_rate_before']:.2%} → {report['missing_rate_after']:.2%}"
        )
...
def apply_robust_imputation(
    df: pd.DataFrame, config: ImputationConfig | None = None
) -> pd.DataFrame:
    """
