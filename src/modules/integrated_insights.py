...
from src.modules.metrics_validator import MetricsValidator, ValidationResult
from src.modules.llm_insights import LLMInsightGenerator

try:
    from src.modules.inventory_optimization_enhanced import EnhancedInventoryOptimizer, InventoryMetrics
...
class IntegratedInsightsGenerator:
    ...
    def generate_validated_insight(
        ...
        logger.debug(f"  ✓ Forecast validated (confidence: {validated_forecast['confidence_level']})")
        
        # STEP 2: Calculate inventory metrics (Module 2)
        logger.debug(f"Calculating inventory metrics...")
        ...
        try:
            ...
            logger.debug(f"  ✓ Inventory metrics calculated")
        ...
        logger.debug(f"Calculating pricing metrics...")
        ...
        try:
            ...
            logger.debug(f"  ✓ Pricing metrics calculated")
        ...
        logger.debug(f"Generating insight...")
        ...
        logger.info(f"✓ Insight generated for {product_id} (method: {insight['method']})")
