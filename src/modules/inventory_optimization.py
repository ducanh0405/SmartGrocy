...
class InventoryOptimizer:
    ...
    def __init__(self, config: InventoryConfig | None = None):
        ...
        logger.info(
            f"Inventory Optimizer initialized with service level: {self.config.service_level:.1%}"
        )

    def calculate_safety_stock(self, demand_std: float, lead_time_days: int | None = None) -> float:
        """
    ...
    def calculate_safety_stock_quantile_based(
        self, q50: float, q95: float, service_level: float = 0.95
    ) -> dict[str, float]:
        """
    ...
    def calculate_modified_economic_order_quantity(
        ...
        if constrained_eoq < traditional_eoq["eoq"]:
            ...
            constraint_explanation = (
                f"Shelf-life constraint applied: max {max_sellable_qty:.0f} units"
            )
        else:
    ...
    def optimize_inventory_from_forecast(
        ...
        shelf_life_days = getattr(
            self.config, "shelf_life_days", 14
        )  # Default 14 days for fresh produce
        ...
        stockout_risk = self._calculate_stockout_probability(
            total_available, avg_daily_demand, demand_std, self.config.lead_time_days
        )
        ...
        return {
            ...
            "recommended_order_quantity": eoq_results.get(
                "recommended_order_quantity", eoq_results["eoq"]
            ),
            "order_frequency_days": eoq_results["order_frequency_days"],
            # Shelf-life parameters
            "shelf_life_days": eoq_results.get("shelf_life_days", shelf_life_days),
            "max_sellable_quantity": eoq_results.get(
                "max_sellable_quantity", avg_daily_demand * shelf_life_days
            ),
    ...
    def _calculate_stockout_probability(
        self, current_inventory: float, avg_demand: float, demand_std: float, time_horizon: int
    ) -> float:
        """
        ...
        for pid in product_ids:
            if store_ids:
                for sid in store_ids:
                    try:
                        result = self.optimize_inventory_from_forecast(forecast_df, pid, sid)
                        results.append(result)
                    except Exception as e:
                        logger.warning(f"Failed to optimize {pid}/{sid}: {e}")
            else:
                try:
                    result = self.optimize_inventory_from_forecast(forecast_df, pid, None)
                    results.append(result)
                except Exception as e:
...
def generate_inventory_report(
    optimization_results: pd.DataFrame, output_path: str | None = None
) -> pd.DataFrame:
    """
    ...
    optimization_results[
        (optimization_results["should_reorder"]) | (optimization_results["stockout_risk"] > 0.1)
...
if __name__ == "__main__":
    ...
    config = InventoryConfig(
        service_level=0.95, ordering_cost=50, holding_cost_rate=0.20, lead_time_days=7, unit_cost=10
    )

    optimizer = InventoryOptimizer(config)

    # Optimize single product
    result = optimizer.optimize_inventory_from_forecast(sample_forecast, "P001", "S001")
