...
class InventoryBacktester:
    ...
    def run_simulation(self, strategy: str = "ml", product_id: str | None = None) -> pd.DataFrame:
        """
        ...
        for _idx, row in data.iterrows():
            ...
            day_result = self.simulate_day(
                inventory, row["actual_demand"], order_qty, safety_stock, age_dist
            )
    ...
    def calculate_kpis(self, simulation_results: pd.DataFrame) -> dict[str, float]:
        ...
        spoilage_rate = (
            (total_spoiled / (total_sold + total_spoiled)) * 100
            if (total_sold + total_spoiled) > 0
            else 0
        )
