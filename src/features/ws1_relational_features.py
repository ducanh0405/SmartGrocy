...
def enrich_relational_features(
    ...
    try:
        ...
        # Check merge success
        matched_products = (
            master_df["MANUFACTURER"].notna().sum() if "MANUFACTURER" in master_df.columns else 0
        )

        logging.info(f"  Merged product data: {original_shape} -> {master_df.shape}")
        logging.info(
            f"  Product matching: {matched_products:,}/{len(master_df):,} rows have product info"
        )
        ...
        if matched_products == 0:
            ...
            logger.warning(
                f"  ⚠️  Only {matched_products/len(master_df)*100:.1f}% products have info - limited coverage"
            )
        else:
            logging.info(
                f"  ✅ Good product data coverage: {matched_products/len(master_df)*100:.1f}%"
            )
    ...
    if "hh_demographic" in dataframes_dict:
        ...
        if "household_key" in master_df.columns and "household_key" in df_hh.columns:
            try:
                ...
                logging.info(
                    f"  Merged household demographics: {original_shape} -> {master_df.shape}"
                )
                ...
                logger.warning(
                    f"Unexpected error in WS1 household merge: {e}. Continuing without household data."
                )
        else:
            logging.info(
                "  Skipping household merge: 'household_key' not found in required dataframes"
            )
    else:
