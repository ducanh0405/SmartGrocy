...
def merge_weather_data(
    ...
    weather_fill_cols = [
        col for col in ["temperature", "precipitation", "humidity"] if col in result.columns
    ]
    if weather_fill_cols:
        result[weather_fill_cols] = result.groupby(store_col)[weather_fill_cols].transform(
            lambda col: col.ffill().bfill()
        )

    null_counts = (
        result[weather_fill_cols].isnull().sum() if weather_fill_cols else pd.Series(dtype=int)
    )
...
def create_weather_features(df: pd.DataFrame, dataset_config: dict | None = None) -> pd.DataFrame:
    ...
    df_result["rain_lag_1d"] = (
        df_result.groupby([product_col, store_col])["is_rainy"].shift(24).astype("float32")
    )

    # 4. Temperature change (24-hour change)
    df_result["temp_change_24h"] = (df_result["temperature"] - df_result["temp_lag_1d"]).astype(
        "float32"
    )
