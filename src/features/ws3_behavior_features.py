...
def _process_clickstream_logs(df_events):
    ...
    if "timestamp" in df_events.columns:
        df_events["timestamp"] = pd.to_datetime(
            df_events["timestamp"], unit="ms"
        )  # Assume timestamp is in ms
...
def _create_user_features(df_logs):
    ...
    user_features = df_logs.pivot_table(
        index="visitorid", columns="event", aggfunc="size", fill_value=0
    )
    ...
    user_features["rate_view_to_cart"] = user_features["total_addtocart"] / (
        user_features["total_views"] + 1e-6
    )

    # Add to cart -> Purchase rate
    user_features["rate_cart_to_buy"] = user_features["total_transactions"] / (
        user_features["total_addtocart"] + 1e-6
    )

    # View -> Purchase rate (overall conversion rate)
    user_features["rate_view_to_buy"] = user_features["total_transactions"] / (
        user_features["total_views"] + 1e-6
    )
    ...
    if "timestamp" in df_logs.columns:
        ...
        time_stats["session_duration_days"] = (
    ...
    if "timestamp" in df_logs.columns:
        ...
        user_features["days_since_last_action"] = (
            latest_timestamp - last_interaction
        ).dt.total_seconds() / (60 * 60 * 24)
