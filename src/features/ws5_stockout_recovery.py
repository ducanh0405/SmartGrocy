...
def recover_latent_demand(df: pd.DataFrame, dataset_config: dict | None = None) -> pd.DataFrame:
    ...
    if config.get("temporal_unit") == "hour" and not pd.api.types.is_datetime64_any_dtype(
        df_result[time_col]
    ):
        try:
    ...
    for _, group_df in df_result.groupby(group_columns, sort=False):
        group_sorted = (
            group_df.sort_values(time_col).reset_index().rename(columns={"index": "__orig_index__"})
        )

        if "_hour_of_day" not in group_sorted.columns and pd.api.types.is_datetime64_any_dtype(
            group_sorted[time_col]
        ):
...
def add_stockout_features(df: pd.DataFrame, dataset_config: dict | None = None) -> pd.DataFrame:
    ...
    expected_demand_window = (
        24 if config.get("temporal_unit") == "hour" else max(config.get("rolling_windows", [4]))
    )
    ...
    for _, group_df in df_result.groupby(group_columns, sort=False):
        ...
        group_copy["time_since_last_stockout"] = _calculate_time_since_last_event(
            stockout_mask, forward=False
        )
        group_copy["time_to_next_stockout"] = _calculate_time_since_last_event(
            stockout_mask, forward=True
        )

        stockout_freq_series = (
            stockout_mask.astype(int).rolling(window=frequency_window, min_periods=1).sum()
        )
        ...
        if target_col in group_copy.columns:
            group_copy["expected_demand"] = (
                group_copy[target_col].rolling(window=expected_demand_window, min_periods=3).mean()
            )
        else:
        ...
        if "latent_demand" in group_copy.columns and target_col in group_copy.columns:
            ...
            group_copy.loc[normal_positions, "stockout_severity"] = (
                group_copy.loc[normal_positions, target_col] / expected.loc[normal_positions]
            )
        ...
        if groups_processed % 100 == 0:
            logger.info(
                f"WS5: Processed stockout features for {groups_processed} product-store groups..."
            )
    ...
    logger.info(
        f"WS5: Stockout features complete - processed {groups_processed} groups (Added {n_new_features} features)"
    )
