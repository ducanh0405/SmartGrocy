...
def create_master_grid(df: pd.DataFrame, dataset_config: dict = None) -> pd.DataFrame:
    ...
    if has_stockout:
        ...
        logger.info(
            f"WS0-Config: Non-stockout dataset - filtered zeros: {len(agg_df)} records remain"
        )
    ...
    if estimated_memory_gb > 2.0:
        logger.warning(
            f"WS0-Config: Large grid detected! Estimated memory: {estimated_memory_gb:.2f} GB"
        )
...
def create_master_grid_polars(df_agg: pl.DataFrame) -> pl.DataFrame:
    ...
    logger.info(
        f"WS0-Polars: Grid: {n_products:,}×{n_stores:,}×{n_weeks:,} = {n_products*n_stores*n_weeks:,} combinations"
    )

    # Create grid using cross joins
    product_store_grid = all_products.join(all_stores, how="cross")
    full_grid = product_store_grid.join(all_weeks, how="cross").select(
    ...
    master_df = (
        full_grid.join(df_agg, on=["PRODUCT_ID", "STORE_ID", "WEEK_NO"], how="left")
...
def prepare_master_dataframe_polars(raw_transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    is_sorted = (
        master_df_pd.groupby(["PRODUCT_ID", "STORE_ID"])["WEEK_NO"]
    ...
    logger.info(
        f"WS0-Polars: Complete: {master_df_pd.shape[0]:,} rows, {master_df_pd.shape[1]} columns"
    )
    return master_df_pd
...
def aggregate_to_weekly_pandas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sales_col = (
        config["required_columns"][3] if len(config["required_columns"]) > 3 else "SALES_VALUE"
    )
...
def create_master_grid_pandas(df_agg: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        f"WS0-Pandas: Grid: {'×'.join([f'{count:,}' for count in entity_counts])}×{len(time_values):,} = {total_combinations:,} combinations"
    )
...
def prepare_master_dataframe_pandas(raw_transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        "sale_amount" in raw_transactions.columns
        and "sales_quantity" not in raw_transactions.columns
    ):
        raw_transactions = raw_transactions.copy()
...
def prepare_master_dataframe_optimized(raw_transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.info(
        "WS0-Optimized: Starting OPTIMIZED master dataframe preparation (ACTIVE pairs only + FORCE pandas)"
    )
    ...
    logger.info(
        f"WS0-Optimized: Processing {len(raw_transactions):,} transactions (FORCE pandas mode)"
    )
...
def prepare_master_dataframe(raw_transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    is_large_dataset = len(raw_transactions) > 1_000_000 or PERFORMANCE_CONFIG.get(  # > 1M rows
        "force_pandas_for_large_data", False
    )

    use_polars = (
        PERFORMANCE_CONFIG.get("use_polars", True)
        and POLARS_AVAILABLE
        and not is_large_dataset  # Force pandas for large datasets
    )
...
def create_optimized_master_grid(df_weekly: pd.DataFrame) -> pd.DataFrame:
    ...
    if config["temporal_unit"] == "week":
        ...
        logger.info(
            f"WS0-Optimized: ACTIVE pairs only: {n_active_pairs:,} pairs × {n_periods:,} periods = {total_optimized:,} rows"
        )
        logger.info(f"WS0-Optimized: Memory reduction: {memory_reduction:.1f}%")
    else:
        logger.info(
            f"WS0-Optimized: ACTIVE pairs only: {n_active_pairs:,} pairs × {n_periods:,} periods"
        )
    ...
    master_df = optimized_grid.merge(df_weekly, on=[product_col, store_col, time_col], how="left")
    ...
    logger.info(
        f"WS0-Optimized: Grid complete: {len(master_df):,} rows ({filled_rows:,} zero-filled, {(filled_rows/len(master_df)*100):.1f}%)"
    )
...
def create_master_grid_pandas_chunked(df_weekly: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        f"WS0-Pandas-Chunked: Grid: {n_products:,}×{n_stores:,}×{n_weeks:,} = {total_combinations:,} combinations"
    )
    ...
    product_store_combinations = pd.MultiIndex.from_product(
        [all_products, all_stores], names=["PRODUCT_ID", "STORE_ID"]
    )
    ...
    for i in range(0, len(product_store_combinations), chunk_size):
        ...
        chunk_result = chunk_grid.merge(
            df_weekly, on=["PRODUCT_ID", "STORE_ID", "WEEK_NO"], how="left"
        )
        ...
        logger.info(
            f"WS0-Pandas-Chunked: Processed chunk {len(chunks)}/{(len(product_store_combinations) // chunk_size) + 1}"
        )
    ...
    filled_rows = len(master_df) - len(df_weekly)
    logger.info(
        f"WS0-Pandas-Chunked: Grid complete: {len(master_df):,} rows ({filled_rows:,} zero-filled)"
    )
    return master_df
